...
fn collapse_items<'a, T: EntryLike>(cite: &mut SpeculativeCiteRender<'a, '_, T>) {
    ...
    // https://github.com/citation-style-language/test-suite/issues/36). Styles
    // like APA that sort and only set `collapse` would otherwise fall back to
    // the layout delimiter, yielding "(Smith 2000; 2001)" instead of
    // "(Smith 2000, 2001)". Unsorted styles keep using the layout delimiter.
...
fn substitute_subsequent_authors(
    ...
    if let Some(subs) = subs {
        ...
        for item in items.iter_mut() {
            let ec = &mut item.0;
            let Some(names_elem) = ec.find_names() else {
                continue;
            };
            ...
            match rule {
                SubsequentAuthorSubstituteRule::CompleteAll => {
                    ...
                    {
                        let names = ec.find_names_mut().unwrap();
